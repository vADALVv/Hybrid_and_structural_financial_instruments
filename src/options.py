...
class BasketOption:
    ...
    def ju_taylor_approximation_call(self):
        # Корреляционная матрица
        rho_matrix = self.corr

        # Расчет взвешенной волатильности корзины
        sigma_squared = self.vol ** 2
        basket_variance = np.dot(
            self.weights, 
            np.dot(rho_matrix * np.outer(self.vol, self.vol), self.weights)
        )
        basket_sigma = np.sqrt(basket_variance)

        # Расчет текущей цены корзины активов
        basket_price = np.sum(self.weights * self.prices)

        d1 = (np.log(basket_price / self.strike) + (0.5 * basket_sigma**2) * self.time) / (basket_sigma * np.sqrt(self.time))
        d2 = d1 - basket_sigma * np.sqrt(self.time)

        call_price = basket_price * norm.cdf(d1) - self.strike * norm.cdf(d2)
